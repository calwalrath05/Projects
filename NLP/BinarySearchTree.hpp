...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  //           in the tree rooted at 'node' or a null pointer if the 
  //           tree is empty.
  // NOTE: This function must be tail recursive.
  ...
  static bool check_sorting_invariant_impl(const Node *node, Compare less) {
    bool left_node_compliant, right_node_compliant;
    bool left_obey = true;
    bool right_obey = true;
    if (!node) {
      return true;
    }
    //if (node->left) {
    if(node->left) {
      left_obey = less(node->left->datum, node->datum);
    }
      left_node_compliant = left_obey && check_sorting_invariant_impl
      (max_element_impl(node->left), less);
    //}
    //if (node->right) {
    if(node->right) {
      right_obey = less(node->datum, node->right->datum);
    }
      right_node_compliant = right_obey && check_sorting_invariant_impl
      (min_element_impl(node->right), less);
    //}
    return left_node_compliant && right_node_compliant;

/*
    bool left_node_compliant, right_node_compliant;
    if (node->left) {
      left_node_compliant = less(node->left, node);
    }
    else {
      left_node_compliant = true;
    }
    if (node->right) {
      right_node_compliant = less(node, node->right);
    }
    else {
      right_node_compliant = true;
    }
    return left_node_compliant && right_node_compliant && 
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
    ...
    //if val > datum
    //the answer cannot be the root
    if (less(node->datum, val)) {
      return min_greater_than_impl(node->right, val, less);
    }
    //if val < datum
    else if (less(val, node->datum)) {
      //go left if there is a left subtree
      if (node->left) {
        //if the max of the left subtree is <= val, the answer is the root node
    ...
    //if val = datum
      ...
      //go to the greatest value in that subtree
      return min_element_impl(node->right);

      //return min_greater_than_impl(max_element_impl(node), val, less);
    }
  }
